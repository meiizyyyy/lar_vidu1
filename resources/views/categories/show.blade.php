...
@section('content')
    <div class="container-fluid d-flex justify-content-center">
        <div class="row mt-5">
            <h1 class="mt-2 mb-2 justify-content-center d-flex">{{ $category->name }}</h1>

            @if ($products->isEmpty())
                <p class="d-flex justify-content-center">Không có sản phẩm nào trong danh mục này.</p>
            @else
                @foreach ($products as $product)
                    <div class="col-sm-6 col-md-3 mb-4">
                        <a href="{{ route('products.show', $product->product_id) }}" class="product-card h-100 d-block">
                            <img src="{{ $product->image_url ? asset('storage/' . $product->image_url) : asset('placeholder.jpg') }}"
                            ...
                            <div class="card-body d-flex flex-column">
                                ...
                                <div class="d-flex justify-content-between pb-2">
                                    ...
                                    <div class="d-flex flex-column text-right">
                                        ...
                                        <h6 class="mb-0">
                                            {{ $product->category ? $product->category->name : 'Không có danh mục' }}</h6>
                                    </div>
                                </div>
